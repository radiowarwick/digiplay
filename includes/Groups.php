...
class Groups{
	...
	public static function get_by_user($user) { return DigiplayDB::select("groups.*,usersgroups.* FROM groups INNER JOIN usersgroups ON usersgroups.groupid = groups.id WHERE usersgroups.userid = ".$user->get_id()." ORDER BY groups.id", "Group", true); }
