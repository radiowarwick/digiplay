<?php
class Sustainer {
	public function get_total_tracks() { return DigiplayDB::select("COUNT(audioid) FROM audioplaylists WHERE playlistid = 0;"); }

	public function get_total_length() { return DigiplayDB::select("SUM(length_smpl) FROM audio INNER JOIN audioplaylists ON (audio.id = audioplaylists.audioid) WHERE audioplaylists.playlistid = 0;") / 44100; }
