;(function($, window, document, undefined) {

"use strict";

/* *****************************************************************************
 * Private functions and variables
 */

function _getIcon(opts, type){
	return opts.map[type];
}

$.ui.fancytree.registerExtension("glyphicon", {
	// Default options for this extension.
	options: {
		prefix: "glyphicon glyphicon-",
		extra: null,
		map: {
			doc: "file",
			docOpen: "file",
			checkbox: "unchecked",
			checkboxSelected: "check",
			checkboxUnknown: "expand",
			error: "warning-sign",
			expanderClosed: "chevron-right",
			expanderLazy: "chevron-right",
			// expanderLazy: "icon-refresh icon-spin",
			expanderOpen: "chevron-down",
			folder: "folder-close",
			folderOpen: "folder-open",
			aw_set: "th",
			showplan: "tasks",
			jingle: "volume-up",
			advert: "bullhorn",
			prerec: "registration-mark",
			script: "file",
			music: "music",
			loading: "refresh rotate"
			// loading: "icon-spinner icon-spin"
		},
		icon: null // TODO: allow callback here
	},
	// Overide virtual methods for this extension.
	// `this`       : is this extension object
	// `this._super`: the virtual function that was overriden (member of prev. extension or Fancytree)
	treeInit: function(ctx){
		var tree = ctx.tree;
		this._super(ctx);
		tree.$container.addClass("fancytree-ext-glyphicon");
	},
	nodeRenderStatus: function(ctx) {
		var icon, span,
			node = ctx.node,
			opts = ctx.options.glyphicon,
			map = opts.map,
			prefix = opts.prefix
			// $span = $(node.span)
			;

		this._super(ctx);

		if( node.isRoot() ){
			return;
		}
		if( node.hasChildren() !== false ){
			span = $("span.fancytree-expander", node.span).get(0);
			if( span ){
				if( node.expanded ){
					icon = "expanderOpen";
				}else if( node.lazy && node.children == null ){
					icon = "expanderLazy";
				}else{
					icon = "expanderClosed";
				}
				// icon = node.expanded ? "expanderOpen" : (node.lazy && node.children == null) ? "expanderLazy" : "expanderClosed";
				span.className = "fancytree-expander " + prefix + map[icon];
			}
		}
		span = $("span.fancytree-checkbox", node.span).get(0);
		if( span ){
			icon = node.selected ? "checkboxSelected" : (node.partsel ? "checkboxUnknown" : "checkbox");
			span.className = "fancytree-checkbox " + map[icon];
		}
		span = $("span.fancytree-icon", node.span).get(0);
		// if( callback && callback(node))
		if( span ){
			if( node.folder ){
				icon = node.expanded ? _getIcon(opts, "folderOpen") : _getIcon(opts, "folder");
			}else{
				icon = _getIcon(opts, node.data.type);
			}
			span.className = "fancytree-icon " + prefix + icon;
		}
	},
	nodeSetStatus: function(ctx, status, message, details) {
		var span,
			opts = ctx.options.glyphicon,
			node = ctx.node;

		this._super(ctx, status, message, details);

		if(node.parent){
			span = $("span.fancytree-expander", node.span).get(0);
		}else{
			span = $("span.fancytree-statusnode-wait, span.fancytree-statusnode-error", node.span).find("span.fancytree-expander").get(0);
		}
		if( status === "loading"){
			// $("span.fancytree-expander", ctx.node.span).addClass(_getIcon(opts, "loading"));
			span.className = "fancytree-expander " + opts.prefix + _getIcon(opts, "loading");
		}else if( status === "error"){
			span.className = "fancytree-expander " + opts.prefix + _getIcon(opts, "error");
		}
	}
});
}(jQuery, window, document));